...
describe('files', () => runOnAndOff((thing) => {
  ...
  it.only('add recursively test', function () {
  ...
  describe('ls', () => {
    before(() => {
      return ipfs('files add -r test/fixtures/test-data/recursive-get-dir')
        .then(console.log.bind(console, 'files added:'))
    })

    it('prints added files', function () {
      this.timeout(20 * 1000)
      // old: QmYmW4HiZhotsoSqnv2o1oUusvkRM8b9RweBoH7ao5nki2
      return ipfs('ls QmRyExB6Q2DYGuztKfXC3txxDsRKW8QS9ib7XQM7xy16hx')
        .then((out) => {
          expect(out).to.eql(
    ...
    it('adds a header, -v', function () {
      ...
      return ipfs('ls /ipfs/QmRyExB6Q2DYGuztKfXC3txxDsRKW8QS9ib7XQM7xy16hx -v')
        .then((out) => {
          expect(out).to.eql(
            'Hash                                           Size   Name\n' +
            'QmQQHYDwAQms78fPcvx1uFFsfho23YJNoewfLbi9AtdyJ9 123530 blocks/\n' +
    ...
    it('follows a path, ls <subdir>', function () {
      ...
      return ipfs('ls /ipfs/QmRyExB6Q2DYGuztKfXC3txxDsRKW8QS9ib7XQM7xy16hx/init-docs')
        .then((out) => {
          expect(out).to.eql(
    ...
    it('recursively follows folders, -r', function() {
      ...
      return ipfs('ls -r /ipfs/QmRyExB6Q2DYGuztKfXC3txxDsRKW8QS9ib7XQM7xy16hx/init-docs')
        .then(out => {
          console.log('output from ls -r:', out)
